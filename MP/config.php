<?php
    ...
    function unique_multidim_array($array, $key1, $key2,$key3=false) { 
        $temp_array = array();
        $i = 0; 
        if($key3){
            $key_array=array(array(array()));
            foreach($array as $val) { 
                if (isset($key_array[$val[$key1]][$val[$key2]][$val[$key3]])) {
                    echo $val[$key1]." / ". $val[$key2]." / ". $val[$key3]."</br>";
                    $data=$key_array[$val[$key1]][$val[$key2]][$val[$key3]];
                    if($temp_array[$data[1]][0]>$val[0]){
                        $temp_array[$data[1]]=$val;
                        $key_array[$val[$key1]][$val[$key2]][$val[$key3]][0] = $val[0];
                    }
                } else{
                    $key_array[$val[$key1]][$val[$key2]][$val[$key3]] = array($val[0],$i);
                    $temp_array[$i]=$val;
                    $i++; 
            ...
            foreach($array as $val) { 
                if (isset($key_array[$val[$key1]][$val[$key2]])) {
                    $data=$key_array[$val[$key1]][$val[$key2]];
                    if($temp_array[$data[1]][0]>$val[0]){
                        $temp_array[$data[1]]=$val;
                        $key_array[$val[$key1]][$val[$key2]][0] = $val[0];
                    }
                } else{
                    $key_array[$val[$key1]][$val[$key2]] = array($val[0],$i);
                    $temp_array[$i]=$val;
                    $i++; 
                }
            } 
